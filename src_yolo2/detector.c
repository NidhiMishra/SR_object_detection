...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh)
{
    ...
    while(1){
		...
		char saveNum[50];
		sprintf(saveNum, "predictions%d", saveImgNum);
        save_image(im, saveNum);
...
//modified by fzw 20171122
void test_detector_img_for_grasping(char **names, image **alphabet, network net, image im, image imFilter, float thresh, object *RecObects, int *objectNumPerFrame)
{
	...
	float nms = 0.1;// .4;
	//image im = load_image_color(input, 0, 0);
	image sized = resize_image(imFilter, net.w, net.h);
	layer l = net.layers[net.n - 1];
	...
	get_region_boxes(l, 1, 1, thresh, probs, boxes, 0, 0);
	if (nms) do_nms_sort(boxes, probs, l.w*l.h*l.n, l.classes, nms);
	//object_detections(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes);
	draw_detections_test(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes, RecObects, objectNumPerFrame);
	//draw_detections(im, l.w*l.h*l.n, thresh, boxes, probs, names, alphabet, l.classes);
	char filename[50];
	//sprintf(filename,"predictions%d", saveImgNum);
...
//modified by fzw 20170930

void test_detector_img(char **names, image **alphabet, network net, image im, float thresh, object *RecObects, int *objectNumPerFrame)
{
