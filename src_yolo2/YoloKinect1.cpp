...
int main(int argc, char* argv[])
{
	try {
		...
		numObject = "10";
		cfgfile = "D:\\nadineRobot\\i2p_Nadine_Robot\\development\\i2p_perception\\i2p_object_detection\\data\\cfg\\yolo.cfg";
		weightfile = "D:\\nadineRobot\\i2p_Nadine_Robot\\development\\i2p_perception\\i2p_object_detection\\data\\weights\\yolo.weights";
		...
		bool isUseThrift = (atoi(useThrift) != 0);
		isUseThrift = 0;
		if (isUseThrift) {
			...
			KinectUtil *mKinectUtil = new KinectUtil(datacfg, namelist, cfgfile, weightfile, atoi(numObject), ObjectDetectionClient);
			mKinectUtil->run();
		}
		else {
			KinectUtil *mKinectUtil = new KinectUtil(datacfg, namelist, cfgfile, weightfile, atoi(numObject));
			mKinectUtil->run();
		}
